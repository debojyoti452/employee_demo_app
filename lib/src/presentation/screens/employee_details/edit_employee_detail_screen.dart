...
import 'add_employee_detail_screen.dart';
import '../home/cubit/home_cubit.dart';
import 'cubit/employee_details_cubit.dart';

class EditEmployeeDetailScreen extends StatefulWidget {
  static const name = 'edit_employee_detail_screen';

  static const path = '/edit_employee_detail_screen';

  const EditEmployeeDetailScreen({
    super.key,
  ...
  @override
  _EditEmployeeDetailScreenState createState() => _EditEmployeeDetailScreenState();
}

class _EditEmployeeDetailScreenState extends BaseStateWrapper<EditEmployeeDetailScreen> {
  ...
  void onInit() {
    ...
    _cubit.initialize();
    _cubit.initialEmployeeItemModel(widget.employeeModel);
  }
  ...
  Widget onBuild(
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text(context.l10n.editEmployee),
          actions: [
            IconButton(
              onPressed: () {
        ...
        body: BlocConsumer<EmployeeDetailsCubit, EmployeeDetailsState>(
          ...
          listener: (context, state) {
            if (state.appState is CubitSuccess) {
              ...
              if (status.event == 'update_employee') {
                context.read<HomeCubit>().fetchEmployeeList();
                context.pop();
              }

              if (status.event == 'delete_employee') {
                context.read<HomeCubit>().fetchEmployeeList();
                context.pop();
              }
            }
              ...
              child: Column(
                ...
                children: [
                  ...
                  CustomTextFormField(
                    ...
                    initialValue: state.employeeModel.employeeName ?? '',
                  ),
                  verticalSpace(20),
